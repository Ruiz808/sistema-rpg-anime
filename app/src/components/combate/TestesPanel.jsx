...
export default function TestesPanel() {
    ...
    const [profConfig, setProfConfig] = useState(0);
    ...
    const executarRolagem = (nomeTeste, tipoAttr, isSavingThrow = false) => {
        ...
        const detalheCalc = `🎲 Dados: ${stringDados} <br/> 📈 Mod. ${tipoAttr.toUpperCase()} (${modBase}) + Prof (${prof}) + Bônus (${bonus})`;
    ...
    return (
        <div className="testes-panel" style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div className="def-box" style={{ border: '1px solid #00ffcc', background: 'rgba(0, 255, 204, 0.05)' }}>
                <h3 style={{ color: '#00ffcc', marginTop: 0, marginBottom: 10 }}>⚙️ Modificadores da Rolagem</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(120px, 1fr))', gap: 10 }}>
                    ...
                    <div>
                        ...
                        <input className="input-neon" type="number" value={profConfig} onChange={e => setProfConfig(e.target.value)} />
                    </div>
                    <div>
            ...
            <div className="def-box">
                <h3 style={{ color: '#ffcc00', marginTop: 0, marginBottom: 15 }}>🛡️ Saving Throws (Testes de Resistência)</h3>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(150px, 1fr))', gap: 10 }}>
                    {SAVES.map(save => (
                        <button 
                            key={save.id} 
                            className="btn-neon" 
                            onClick={() => executarRolagem(save.label, save.attr, true)}
                        ...
                        >
                            ...
                            <span style={{ fontSize: '0.7em', color: '#fff', marginTop: 4 }}>Mod: +{getModificadorDoisDigitos(minhaFicha[save.attr]?.base)}</span>
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="def-box">
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10, marginBottom: 15 }}>
                    <h3 style={{ color: '#0088ff', margin: 0 }}>🎯 Testes de Habilidade</h3>
                    <input 
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
                    {skillsFiltradas.length > 0 ? skillsFiltradas.map(skill => (
                        <button 
                            key={skill.id} 
                            className="btn-neon" 
                            onClick={() => executarRolagem(skill.label, skill.attr, false)}
                            style={{ borderColor: '#444', color: '#fff', margin: 0, padding: '8px 10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.4)', transition: 'border-color 0.2s' }}
                        ...
                        >
                            ...
                            <span style={{ fontSize: '0.7em', color: '#0088ff', background: 'rgba(0,136,255,0.2)', padding: '2px 6px', borderRadius: 4 }}>
                                {skill.attr.substring(0, 3).toUpperCase()} (+{getModificadorDoisDigitos(minhaFicha[skill.attr]?.base)})
                            </span>
                        </button>
                    )) : (
