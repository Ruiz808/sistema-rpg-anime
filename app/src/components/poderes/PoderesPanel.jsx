...
export default function PoderesPanel() {
    ...
    const handleImageUpload = async (e) => {
        ...
        try {
            ...
            console.error(err);
            alert('Erro ao enviar a imagem para a Forja! Verifique se as regras do Storage estao ativas no Firebase.');
        } finally {
    ...
    const editarPoder = (id) => {
        ...
        if (p.ativa) {
            togglePoder(id);
            alert(`A tecnica [${p.nome}] foi DESATIVADA temporariamente para edicao.`);
        }
    ...
    const vincularArma = (poderId, armaId) => {
        updateFicha((ficha) => {
    ...
    return (
        <div className="poderes-panel" style={{ display: 'flex', gap: '20px', flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            {/* COLUNA ESQUERDA: Menu de Navegacao Vertical */}
            ...
            {/* COLUNA DIREITA: Conteudo (Formulario e Lista) */}
            <div style={{ flex: '1 1 500px', display: 'flex', flexDirection: 'column', gap: '15px' }}>
                ...
                <div className="def-box" ref={formRef} id="form-poder-box">
                    ...
                    {dadosQtd > 0 && <p style={{ color: '#f0f', fontSize: '0.85em', margin: '5px 0 0' }}>Dano: {dadosQtd}d{dadosFaces}{custoPercentual > 0 ? ` | Custo: ${custoPercentual}%` : ''}</p>}
                    ...
                    <div style={{ display: 'flex', gap: 10, marginTop: 10 }}>
                        <button className="btn-neon btn-gold" onClick={salvarNovoPoder} style={{ flex: 1 }}>{poderEditandoId ? 'Salvar Edicao' : `Salvar ${SINGULAR[abaAtual]}`}</button>
                                ...
                                <span style={{ color: '#f0f', display: 'block', marginTop: '4px' }}>{(p.efeitosPassivos || []).map(e => {
                                    ...
                                    return `PASSIVO: [${(e.atributo || '').replace('_', ' ').toUpperCase()}] ${(e.propriedade || '').toUpperCase()}: ${isMult ? 'x' : '+'}${e.valor || 0}`;
                                        ...
                                        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                                            ...
                                            <button className="btn-neon" style={{ borderColor: '#ff0', color: '#ff0', padding: '5px 15px', fontSize: '1em', margin: 0 }} onClick={() => setVincularAberto(vincularAberto === p.id ? null : p.id)}>VINCULAR</button>
                                    ...
                                    {vincularAberto === p.id && (
                                        <div style={{ marginTop: 10, padding: '10px', background: 'rgba(255,255,0,0.05)', borderLeft: '3px solid #ff0', borderRadius: '4px' }}>
                                            <label style={{ color: '#ff0', fontSize: '0.85em', marginBottom: 5, display: 'block' }}>Vincular a Equipamento:</label>
                                            <select
                                                className="input-neon"
                                                value={String(p.armaVinculada || '')}
                                                onChange={e => vincularArma(p.id, e.target.value)}
                                                style={{ margin: 0 }}
                                            >
