...
export default function MapaMundi({ children }) {
    ...
    const [camZoom, setCamZoom] = useState(0.35); // Zoom afastado para caber o novo universo colossal
    const [isDraggingCam, setIsDraggingCam] = useState(false);
    const camDragStart = useRef({ x: 0, y: 0 });

    // 🛣️ FAIXAS ESCALA COLOSSAL (3000px de Universo - Matemática Absoluta)
    const caminhosOrbita = [
        "M 1500 750 C 1950 150, 2700 300, 2700 750 C 2700 1200, 1950 1350, 1500 750 C 1050 150, 300 300, 300 750 C 300 1200, 1050 1350, 1500 750 Z", // Faixa 0 (Terra 0)
        "M 1500 750 C 1995 90, 2775 240, 2775 750 C 2775 1260, 1995 1410, 1500 750 C 1005 90, 225 240, 225 750 C 225 1260, 1005 1410, 1500 750 Z", // Faixa 1 (Vegeta)
        "M 1500 750 C 1905 210, 2625 360, 2625 750 C 2625 1140, 1905 1290, 1500 750 C 1095 210, 375 360, 375 750 C 375 1140, 1095 1290, 1500 750 Z", // Faixa 2 (Namekusei)
        "M 1500 750 C 2040 30, 2850 180, 2850 750 C 2850 1320, 2040 1470, 1500 750 C 960 30, 150 180, 150 750 C 150 1320, 960 1470, 1500 750 Z" // Faixa 3 (Desconhecido)
    ];
    ...
    // 📍 VARIÁVEL 2: POSIÇÕES DO SISTEMA SOLAR (Estrelas centralizadas nos loops)
    const [elementosSolar, setElementosSolar] = useState([
        { "id": "orichalcosA", "tipo": "estrela", "nome": "Orichalcos A", "top": "50%", "left": "30%", "size": "320px" },
        { "id": "orichalcosB", "tipo": "estrela", "nome": "Orichalcos B", "top": "50%", "left": "70%", "size": "320px" },
    ...
    const handleDragMove = (e) => {
        ...
        if (dragIndexCosmo !== null) {
            ...
            setElementosSolar(prev => {
                const novas = [...prev];
                novas[elIndex] = { ...novas[elIndex], top: `${yPercent.toFixed(1)}%`, left: `${xPercent.toFixed(1)}%` };
                return novas;
            });
        }
    };
    ...
    // ==========================================
    // 🌌 TELA 0: SISTEMA SOLAR CÂMERA 3D PURA
    // ==========================================
            ...
            <div className="fade-in" style={{ width: '100%', height: '85vh', background: 'radial-gradient(circle at center, #0a0a1a 0%, #000000 100%)', position: 'relative', overflow: 'hidden', borderRadius: '15px', border: '1px solid #333' }}>
                ...
                >
                    ...
                    <div 
                        ...
                        style={{ 
                            position: 'relative', width: '3000px', height: '1500px', 
                    ...
                    >
                        ...
                        {/* RODOVIA CÓSMICA EM SVG COM OVERFLOW VISIBLE */}
                        <svg viewBox="0 0 3000 1500" style={{ position: 'absolute', top: 0, left: 0, width: '100%', height: '100%', pointerEvents: 'none', zIndex: 1, transformStyle: 'preserve-3d', overflow: 'visible' }}>
                            <defs>
                            ...
                            {caminhosOrbita.map((path, idx) => (
                                <path 
                                    ...
                                    d={path} 
                                    fill="none" 
                                    stroke={idx === 0 ? "rgba(255, 255, 255, 0.7)" : "rgba(255, 255, 255, 0.3)"} 
                        ...
                        {/* PLANETAS E ESTRELAS EM HTML 3D PURO */}
                        {elementosSolar.map(el => {
                            if (el.tipo === 'estrela') {
                                // O transform 'translate(-50%, -50%)' ancora o sol exatamenteno ponto central dele!
                                return (
                                    <div key={el.id} style={{ position: 'absolute', top: el.top, left: el.left, width: 0, height: 0, zIndex: 5, transformStyle: 'preserve-3d' }}>
                                        <div onMouseDown={(e) => handleDragStart(e, el.id, true)} onTouchStart={(e) => handleDragStart(e, el.id, true)} style={{ position: 'absolute', width: el.size, height: el.size, transform: `translate(-50%, -50%) ${billboardTransform}`, transformStyle: 'preserve-3d', pointerEvents: 'auto', border: modoAjuste ? '2px dashed #fff' : 'none', borderRadius: '50%', background: 'radial-gradient(circle at 30% 30%, #ffffff 0%, #ffcc00 40%, #ff5500 100%)', boxShadow: '0 0 150px #ffcc00, 0 0 250px #ff5500, inset -30px -30px 60px rgba(255,50,0,0.8)' }}>
                                        </div>
                                    </div>
                                );
                            }
                            ...
                            if (el.tipo === 'planeta') {
                                return (
                                    <div key={el.id} style={{ position: 'absolute', top: 0, left: 0, width: el.size, height: el.size, zIndex: 10, offsetPath: `path("${caminhosOrbita[el.pathIdx || 0]}")`, offsetRotate: '0deg', animation: `orbitaSempre ${el.tempo} linear infinite`, animationDelay: el.delay, transformStyle: 'preserve-3d', pointerEvents: 'auto' }}>
                                        <div style={{ width: '100%', height: '100%', transform: billboardTransform, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transformStyle: 'preserve-3d' }}>
                                            ...
                                            <span style={{ color: el.color1, position: 'absolute', bottom: '-40px', fontSize: '16px', fontWeight: 'bold', letterSpacing: '2px', textTransform: 'uppercase', whiteSpace: 'nowrap', textShadow: '0 2px 4px rgba(0,0,0,0.9), 0 0 10px rgba(255,255,255,0.5)' }}>{el.nome}</span>
                                        </div>
                                    </div>
                                );
                            }
                            ...
                            if (el.tipo === 'terra') {
                                return (
                                    <div key={el.id} style={{ position: 'absolute', top: 0, left: 0, width: el.size, height: el.size, zIndex: 20, offsetPath: `path("${caminhosOrbita[el.pathIdx || 0]}")`, offsetRotate: '0deg', animation: `orbitaSempre ${el.tempo} linear infinite`, animationDelay: el.delay, transformStyle: 'preserve-3d', pointerEvents: 'auto' }}>
                                        <div style={{ width: '100%', height: '100%', transform: billboardTransform, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', transformStyle: 'preserve-3d' }}>
                                            ...
                                            <span style={{ color: '#4db8ff', position: 'absolute', bottom: '-60px', fontSize: '22px', fontWeight: 'bold', letterSpacing: '5px', textTransform: 'uppercase', textShadow: '0 2px 4px rgba(0,0,0,0.9), 0 0 15px rgba(0,150,255,1)', whiteSpace: 'nowrap', pointerEvents: 'none' }}>{el.nome}</span>
                                        </div>
                                    </div>
                                );
                            }
                ...
                <style dangerouslySetInnerHTML={{__html: `
                    ...
                    /* A lua gira -360 no eixo Z e desfaz os 75 graus pra sempre encarar a tela! */
                    .moon-sphere { 
                        position: absolute; 
