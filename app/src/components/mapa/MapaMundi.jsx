...
export default function MapaMundi({ children }) {
    ...
    // --- DICIONÁRIO DE CORES DO TEU GABARITO ---
    ...
    // --- DETEÇÃO DE PIXEIS (COM DEBUG PARA A CONSOLA) ---
    const detectarReino = (e) => {
        ...
        // 🔥 AVISO NA CONSOLA PARA DESCOBRIRMOS O ERRO 🔥
        if (pixel[3] > 0) { // Só regista se não for transparente
             console.log(`Cor detectada pelo rato: ${corRGB}`);
        }

        let reinoEncontrado = dicionarioCores[corRGB] || null;
        
        if (!reinoEncontrado && pixel[3] > 0) { 
            const [r, g, b] = pixel;
    ...
    useEffect(() => {
        ...
        if (nivelVisao === 'continente' && imgIdMapRef.current) {
            if (imgIdMapRef.current.complete) setTimeout(desenharGabarito, 150); 
    ...
    useEffect(() => {
        ...
        for (let i = 0; i < idData.data.length; i += 4) {
            const r = idData.data[i], g = idData.data[i+1], b = idData.data[i+2], a = idData.data[i+3];
            if (a > 0 && Math.abs(r - tr) < 20 && Math.abs(g - tg) < 20 && Math.abs(b - tb) < 20) {
                    ...
                    >
                        ...
                        <div style={{ position: 'absolute', inset: 0, transform: 'translateZ(160px)', backfaceVisibility: 'hidden', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', pointerEvents: 'none' }}>
                            <svg viewBox="0 0 200 200" style={{ position: 'absolute', width: '140px', height: '140px', filter: 'drop-shadow(0 0 3px rgba(0,255,204,0.8))' }}>
                                <path d="M 12 55 L 18 45 L 30 38 L 42 35 L 55 30 L 70 28 L 85 25 L 100 28 L 115 32 L 125 30 L 135 38 L 142 42 L 140 52 L 132 58 L 122 56 L 115 65 L 105 72 L 95 68 L 82 72 L 68 80 L 52 75 L 40 82 L 25 72 L 15 75 Z" fill="rgba(0,255,204,0.08)" stroke="#00ffcc" strokeWidth="1" />
        ...
        return (
            <div className="fade-in" style={{ width: '100%', height: '65vh', background: '#050508', borderRadius: '10px', border: '1px solid #0088ff', display: 'flex', flexDirection: 'column', overflow: 'hidden', position: 'relative' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.85)', padding: '12px 25px', borderBottom: '1px solid #222', zIndex: 10 }}>
                    ...
                    {/* BOTÃO DE DEBUG */}
                    <button onClick={() => setDebugMode(!debugMode)} style={{ background: debugMode ? '#00ffcc' : 'transparent', color: debugMode ? '#000' : '#00ffcc', border: '1px solid #00ffcc', padding: '5px 10px', borderRadius: '5px', cursor: 'pointer', fontWeight: 'bold' }}>
                        🛠️ {debugMode ? 'ESCONDER GABARITO' : 'VER GABARITO'}
                    </button>
                </div>
                ...
                <div style={{ flex: 1, display: 'flex', justifyContent: 'center', alignItems: 'center', overflow: 'hidden', position: 'relative', width: '100%' }}>
                    ...
                    >
                        ...
                        {/* GABARITO (Fica visível se o Debug estiver ligado!) */}
                        <img 
                            ref={imgIdMapRef} 
                            src={mapaGabarito} 
                            style={{ 
                                position: 'absolute', top: 0, left: 0, width: '100%', height: '100%',
                                display: debugMode ? 'block' : 'none', opacity: 0.8, zIndex: 15
                            }} 
                            alt="Gabarito" 
                        />
                        ...
                        <canvas 
                            ...
                            style={{ 
                                ...
                                display: debugMode ? 'none' : 'block' // Esconde a máscara holográfica no modo debug
                            }} 
                        />

                        <style dangerouslySetInnerHTML={{__html: `
                            .ping-wrapper { position: absolute; transform: translate(-50%, -50%); display: flex; flex-direction: column; align-items: center; z-index: 20; transition: 0.2s; padding: 10px; }
