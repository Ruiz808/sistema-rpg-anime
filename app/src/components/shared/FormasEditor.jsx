                    ...
                    {!editando ? (
                        ...
                        <div style={{ background: 'rgba(20,20,20,0.9)', borderLeft: '4px solid #00ffcc', padding: 15, borderRadius: 4 }}>
                            ...
                            <label style={{ display: 'flex', alignItems: 'center', gap: 8, color: '#aaa', fontSize: '0.85em', cursor: 'pointer', marginBottom: 15 }}>
                                ...
                                Acumula com os atributos normais da arma
                            </label>
                            ...
                            {editando.configs.map((cfg, cIdx) => (
                                <div key={cfg.id} style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid #444', padding: 12, marginBottom: 15, borderRadius: 4 }}>
                                    ...
                                    {/* 🔥 CORREÇÃO: display: grid aplicado aqui 🔥 */}
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px auto', gap: 6, marginBottom: 5 }}>
                                        <select className="input-neon" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tAtrA} onChange={e => handleConfigChange(cIdx, 'tAtrA', e.target.value)}>
                                        ...
                                        <select className="input-neon" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tPropA} onChange={e => handleConfigChange(cIdx, 'tPropA', e.target.value)}>
                                            {PROPRIEDADE_OPTIONS.map(p => <option key={p} value={p}>{p.toUpperCase()}</option>)}
                                        </select>
                                        <input className="input-neon" type="text" placeholder="Valor" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tValA} onChange={e => handleConfigChange(cIdx, 'tValA', e.target.value)} />
                                    ...
                                    {/* 🔥 CORREÇÃO: display: grid aplicado aqui 🔥 */}
                                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 80px auto', gap: 6, marginBottom: 5 }}>
                                        <select className="input-neon" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tAtrP} onChange={e => handleConfigChange(cIdx, 'tAtrP', e.target.value)}>
                                            {ATRIBUTOS_AGRUPADOS.map(g => <optgroup key={g.label} label={g.label}>{g.options.map(a => <option key={a} value={a}>{a.replace('_', ' ').toUpperCase()}</option>)}</optgroup>)}
                                        </select>
                                        <select className="input-neon" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tPropP} onChange={e => handleConfigChange(cIdx, 'tPropP', e.target.value)}>
                                            {PROPRIEDADE_OPTIONS.map(p => <option key={p} value={p}>{p.toUpperCase()}</option>)}
                                        </select>
                                        <input className="input-neon" type="text" placeholder="Valor" style={{ padding: 4, fontSize: '0.8em' }} value={cfg.tValP} onChange={e => handleConfigChange(cIdx, 'tValP', e.target.value)} />
