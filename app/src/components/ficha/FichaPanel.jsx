...
export default function FichaPanel() {
    ...
    function addTrackerCena() {
        ...
        salvarFichaSilencioso();
        setNovoTrackerNome(''); setNovoTrackerValor('');
    }
    ...
    function resetarTrackersCena() {
        if (!window.confirm('A cena terminou? Isso vai zerar todos os seus stacks.')) return;
        updateFicha(f => {
            if (!f.combate || !f.combate.trackers) return;
    ...
    function injetarAnomalia() {
        ...
        salvarFichaSilencioso(); setValorInjecao('');
        setShowAbsorverMsg(`✨ Evolução Concluída! Valor de +${val} injetado!`);
    ...
    return (
        <div className="ficha-panel">
            <div className="def-box" style={{ position: 'relative', overflow: 'hidden', border: isGrand ? '2px solid #ff003c' : '', boxShadow: isGrand ? '0 0 30px rgba(255, 0, 60, 0.3), inset 0 0 50px rgba(255, 204, 0, 0.1)' : '' }}>
                ...
                <button className="btn-neon btn-gold" onClick={salvarBio} style={{ marginTop: 15, width: '100%', position: 'relative', zIndex: 1, backgroundColor: salvandoBio ? 'rgba(0, 255, 100, 0.2)' : undefined, borderColor: salvandoBio ? '#00ffcc' : undefined, color: salvandoBio ? '#fff' : undefined }}>
            ...
            {/* 🔥 PAINÉIS DINÂMICOS DAS VERTENTES DA MATRIX 🔥 */}
            ...
            {/* 3. MATRIZ UTILITÁRIA (CÓPIA/SUPORTE) */}
            {showUtilitario && (
                <div className="def-box fade-in" style={{ marginTop: 15, background: 'rgba(255, 204, 0, 0.05)', border: '1px solid #ffcc00', boxShadow: '0 0 15px rgba(255, 204, 0, 0.2)' }}>
                    ...
                    <div style={{ display: 'flex', gap: '15px', alignItems: 'center', flexWrap: 'wrap', marginBottom: '15px' }}>
                        <input className="input-neon" type="text" placeholder="Nome do Poder Copiado" value={novaCopiaNome} onChange={e => setNovaCopiaNome(e.target.value)} style={{ flex: '1 1 200px', borderColor: '#ffcc00', color: '#fff', margin: 0 }} />
                        <input className="input-neon" type="text" placeholder="Efeito / Buff (Ex: +2000 Dano)" value={novaCopiaEfeito} onChange={e => setNovaCopiaEfeito(e.target.value)} style={{ flex: '1 1 200px', borderColor: '#ffcc00', color: '#fff', margin: 0 }} />
                        <button className="btn-neon" style={{ flex: '0 0 auto', borderColor: '#ffcc00', color: '#ffcc00', margin: 0, padding: '10px 20px', fontWeight: 'bold', background: 'rgba(255, 204, 0, 0.1)' }} onClick={addCopiaAtiva}>+ ARMAZENAR</button>
                    </div>
            ...
            {/* 4. MARCADORES DE CENA (ACUMULATIVOS DE COMBATE E ABSORÇÃO) */}
            {showMarcadoresCena && (
                <div className="def-box fade-in" style={{ marginTop: 15, background: 'rgba(255, 136, 0, 0.05)', border: '1px solid #ff8800', boxShadow: '0 0 15px rgba(255, 136, 0, 0.2)' }}>
                    ...
                    <p style={{ color: '#aaa', fontSize: '0.85em', margin: '0 0 15px 0' }}>Adicione os stacks em tempo real durante o combate.</p>
                            ...
                            {trackersCena.map(tracker => (
                                <div key={tracker.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: 'rgba(0,0,0,0.5)', padding: '10px', borderRadius: '5px', borderLeft: '3px solid #ff8800' }}>
                                    <div style={{ flex: 1 }}>
                                        ...
                                        <div style={{ color: '#ffcc00', fontSize: '1.1em', marginTop: '4px' }}>Bônus Total na Cena: <strong style={{ color: '#0f0' }}>+{(tracker.stacks * tracker.valor)}</strong></div>
                                    </div>
            ...
            {/* 5. FORJA UNIVERSAL DE CALAMIDADE (APENAS ACUMULATIVO DE ABSORÇÃO) */}
            {showForjaCalamidade && (
                <div className="def-box fade-in" style={{ marginTop: 15, background: 'rgba(0, 204, 255, 0.05)', border: '1px solid #00ccff', boxShadow: '0 0 15px rgba(0, 204, 255, 0.2)' }}>
                    <h3 style={{ color: '#00ccff', marginBottom: 10, textShadow: '0 0 5px #00ccff' }}>🌌 Forja de Calamidade (Pós-Luta)</h3>
                    <p style={{ color: '#aaa', fontSize: '0.85em', margin: '0 0 15px 0' }}>Calcule o valor final da sua habilidade e escolha onde ele será injetado permanentemente.</p>
            ...
            <div className="def-box" style={{ marginTop: 15 }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 10 }}>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Base {buffsDano._hasBuff.mbase && <span style={{ color: '#0f0', fontSize: '0.8em' }}>(Buff: +{buffsDano.mbase.toFixed(2)})</span>}</label><input className="input-neon" type="number" step="0.01" value={dmBase} onChange={e => setDmBase(e.target.value)} /></div>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Potencial</label><input className="input-neon" type="number" step="0.01" value={dmPotencial} onChange={e => setDmPotencial(e.target.value)} /></div>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Geral {buffsDano._hasBuff.mgeral && <span style={{ color: '#0f0', fontSize: '0.8em' }}>(Buff: +{buffsDano.mgeral.toFixed(2)})</span>}</label><input className="input-neon" type="number" step="0.01" value={dmGeral} onChange={e => setDmGeral(e.target.value)} /></div>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Formas {buffsDano._hasBuff.mformas && <span style={{ color: '#0f0', fontSize: '0.8em' }}>(Buff: +{buffsDano.mformas.toFixed(2)})</span>}</label><input className="input-neon" type="number" step="0.01" value={dmFormas} onChange={e => setDmFormas(e.target.value)} /></div>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Absoluto {buffsDano._hasBuff.mabs && <span style={{ color: '#0f0', fontSize: '0.8em' }}>(Buff: +{buffsDano.mabs.toFixed(2)})</span>}</label><input className="input-neon" type="number" step="0.01" value={dmAbsoluto} onChange={e => setDmAbsoluto(e.target.value)} /></div>
                    <div><label style={{ color: '#aaa', fontSize: '0.85em' }}>Mult Unico {buffsDano.munico.length > 0 && <span style={{ color: '#0f0', fontSize: '0.8em', textShadow: '0 0 5px rgba(0,255,0,0.5)' }}>(Buff: x{buffsDano.munico.join(' e x')})</span>}</label><input className="input-neon" type="text" value={dmUnico} onChange={e => setDmUnico(e.target.value)} /></div>
                ...
                <button className="btn-neon btn-gold" onClick={salvarMultiplicadores} style={{ marginTop: 15, width: '100%' }}>SALVAR MULTIPLICADORES</button>
            </div>
            <TabelaPrestigio />
        </div>
    );
